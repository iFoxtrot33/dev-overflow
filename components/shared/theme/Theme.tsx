  ...
  return (
    <Menubar>
      <MenubarMenu>
        ...
        <MenubarContent className="absolute -right-12 mt-3 min-w-[120px] cursor-pointer rounded border py-2 dark:border-dark-400 dark:bg-dark-300">
          {themes.map((item) => (
            <MenubarItem
              key={item.value}
              className="flex cursor-pointer items-center gap-4 px-2.5 py-2 focus:bg-primary-100  dark:focus:bg-dark-400"
              onClick={() => {
                setMode(item.value);
